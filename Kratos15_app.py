...
# With Upload Logs, Charts, Live Map & Geo-IP Layers
# =================================================
...
with tab2:
    ...
    med_layer = folium.FeatureGroup(name="🟠 Medium Severity").add_to(m)
    ...
    for _, row in filtered_alerts.iterrows():
        tooltip = (f"Severity: {row['severity']} | "
                   f"Src: {row['source_ip']} → {row['dest_ip']}")
        marker = folium.Marker(
            ...
            tooltip=tooltip,
            icon=folium.Icon(color=severity_colors[row["severity"]],
    ...
    if not filtered_alerts.empty:
        heat_data = filtered_alerts[["lat", "lon"]].dropna().values.tolist()
        HeatMap(heat_data, radius=15, blur=10, name="🌍 Attack Heatmap").add_to(m)
