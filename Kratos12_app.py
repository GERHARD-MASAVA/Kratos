...
# With Upload Logs, Charts & Interactive Live Map
# =================================================
...
with tab1:
    ...
    if "timestamp" in logs.columns and logs["timestamp"].notna().any():
        ...
        if not hourly_counts.empty:
            st.line_chart(hourly_counts, height=250)
        else:
...
# ================================================
# 🗺️ Live Map Tab (Interactive)
# ================================================
with tab2:
    st.subheader("Geo-IP Attack Map (Interactive)")

    m = folium.Map(location=[20,0], zoom_start=2)

    # Separate layers per severity
    low_layer = folium.FeatureGroup(name="Low Severity").add_to(m)
    medium_layer = folium.FeatureGroup(name="Medium Severity").add_to(m)
    high_layer = folium.FeatureGroup(name="High Severity").add_to(m)

    for _, row in filtered_alerts.iterrows():
        icon_color = severity_colors[row["severity"]]
        layer = {"Low": low_layer, "Medium": medium_layer, "High": high_layer}[row["severity"]]
        ...
        folium.Marker(
            ...
            tooltip=f"{row['severity']} threat from {row['source_ip']}",
