...
# -----------------------
# Synthetic logs generator (fallback)
# -----------------------
def generate_synthetic_logs(n=500):
    ...
    df = pd.DataFrame({
        ...
        "source_ip": np.random.choice(
            [
                "8.8.8.8", "1.1.1.1", "77.88.55.77", "185.199.110.153",
                "41.90.5.2", "200.89.75.5", "103.21.244.0", "196.25.1.200"
            ], n
        ),
...
# Sidebar controls
# -----------------------
st.sidebar.title("⚙️ Kratos Controls")
uploaded_file = st.sidebar.file_uploader("Upload log file (CSV/JSON)", type=["csv","json"])

playback_speed = st.sidebar.selectbox("⏱️ Playback Speed (seconds per step)", [2, 5, 10, 30], index=1)
...
if st.session_state.is_playing and st.session_state.hour_pos < total_hours:
...
with tab1:
    ...
    if severity_filter != "All":
        df_view = df_view[df_view["severity"] == severity_filter]
