...
playback_auto = st.sidebar.checkbox("▶️ Auto-play timeline", value=False)

# 🔑 News keyword input
news_keyword = st.sidebar.text_input(
    "News search keyword", 
    value="cyber attack",
    help="Enter keyword (e.g., ransomware, Kenya, APT group)"
)

# ⏱️ News refresh interval
news_refresh_interval = st.sidebar.selectbox(
    "News refresh interval (minutes)", [1, 5, 10, 30], index=1
)
...
# -----------------------
IPINFO_TOKEN = "fe4a295a30fcdc"  # <<-- paste your token here
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Dashboard","🗺️ Live Map","📜 Logs","🛡️ Countermeasures","📰 News"])
...
with tab5:
    ...
    st.caption(f"Showing latest news for: **{news_keyword}**")

    # Auto-refresh every X minutes
    st.experimental_autorefresh(interval=news_refresh_interval * 60 * 1000, key="news_refresh")

    try:
        url = f"https://newsapi.org/v2/everything?q={news_keyword}&sortBy=publishedAt&apiKey=5eb57893abfb4d54b2c2036ee2a21991"
        ...
        if data.get("status") == "ok" and data.get("articles"):
            for art in data["articles"][:10]:
                ...
                st.caption(art["source"]["name"] + " | " + art["publishedAt"])
                st.write(art["description"])
                st.markdown(f"[Read more]({art['url']})")
                st.divider()
        else:
            st.info("No news found for this keyword.")
    except Exception as e:
        st.error(f"News API fetch failed: {e}")
