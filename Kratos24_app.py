...
severity_weights = {"Low":1,"Medium":3,"High":5}  # for heatmap weighting
...
def real_geolocate(ip):
    try:
        ...
        if "loc" in data:
            ...
            flag = "".join([chr(127397 + ord(c)) for c in country]) if len(country) == 2 else "🏳️"
            return (lat, lon, country, flag)
        else:
            return (0, 0, "Unknown", "🏳️")
    except Exception as e:
        print(f"Geo-IP lookup failed for {ip}: {e}")
        return (0, 0, "Unknown", "🏳️")

alerts[["lat","lon","country","flag"]] = alerts["source_ip"].apply(
    lambda ip: pd.Series(real_geolocate(ip))
)
...
with tab1:
    ...
    if df_view.empty:
        ...
        for _, r in df_view.sort_values("timestamp", ascending=False).iterrows():
            st.write(
                f"[{r['timestamp']}] {r['flag']} {r['country']} | **{r['severity']}** | "
                f"{r['source_ip']} → {r['dest_ip']} | logins={r['failed_logins']} bytes={r['bytes_sent']}"
            )
    ...
    for _, r in visible_alerts.iterrows():
        if r["lat"] != 0 and r["lon"] != 0:
            folium.CircleMarker(
                ...
                fill=True,
                fill_opacity=0.8,
                popup=(f"<b>{r['severity']}</b><br>{r['flag']} {r['country']}<br>{r['source_ip']} → {r['dest_ip']}"),
                tooltip=f"{r['severity']} | {r['source_ip']}"
            ).add_to(m)

    if not visible_alerts.empty:
        heat_points = [
            [r["lat"], r["lon"], severity_weights.get(r["severity"],1)]
            for _, r in visible_alerts.iterrows() if r["lat"] != 0 and r["lon"] != 0
        ]
        if heat_points:
            HeatMap(heat_points, radius=12, blur=10, max_zoom=6).add_to(m)
